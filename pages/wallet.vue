<script setup lang="ts">
const { wallet } = useFinance()

const search = ref('')
const exportOpen = ref(false)
const filterOpen = ref(false)
const selectedDate = ref('')
const selectedDateApplied = ref('')
const typeFilter = ref<'All' | 'Earning' | 'Withdraw'>('All')
const typeDraft = ref<'All' | 'Earning' | 'Withdraw'>('All')

const visibleTransactions = computed(() => {
  const query = search.value.trim().toLowerCase()
  return wallet.value.filter((item) => {
    const matchesType = typeFilter.value === 'All' || item.type === typeFilter.value
    const matchesDate = !selectedDateApplied.value || item.date.toLowerCase().includes(selectedDateApplied.value.trim().toLowerCase())
    const matchesQuery = !query || [item.type, item.orderId, item.date].some(value => value.toLowerCase().includes(query))
    return matchesType && matchesDate && matchesQuery
  })
})

const openFilter = () => {
  typeDraft.value = typeFilter.value
  selectedDate.value = selectedDateApplied.value
  filterOpen.value = !filterOpen.value
  exportOpen.value = false
}

const applyFilter = () => {
  typeFilter.value = typeDraft.value
  selectedDateApplied.value = selectedDate.value
  filterOpen.value = false
}

const clearFilter = () => {
  typeDraft.value = 'All'
  typeFilter.value = 'All'
  selectedDate.value = ''
  selectedDateApplied.value = ''
  filterOpen.value = false
}

const downloadFile = (content: string, filename: string, type: string) => {
  if (!import.meta.client) {
    return
  }
  const blob = new Blob([content], { type })
  const url = URL.createObjectURL(blob)
  const link = document.createElement('a')
  link.href = url
  link.download = filename
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
  URL.revokeObjectURL(url)
}

const exportCsv = () => {
  const rows = visibleTransactions.value.map(item => [item.type, item.orderId, item.date, item.amount, item.balance])
  downloadFile(
    [['Type', 'Order Id', 'Date', 'Amount', 'Balance'], ...rows].map(row => row.join(',')).join('\n'),
    'wallet.csv',
    'text/csv;charset=utf-8;',
  )
  exportOpen.value = false
}

const amountLabel = (amount: number) => `${amount >= 0 ? '+' : '-'}GHC ${Math.abs(amount)}`

useHead({
  title: 'Wallet',
})
</script>

<template>
  <section class="dashboard-card">
    <h1 class="wallet-page-title">Wallet</h1>

    <div class="section-bar">
      <h2 class="section-title">Transaction</h2>
      <div class="toolbar">
        <label class="search-box">
          <svg viewBox="0 0 24 24" fill="none">
            <path d="m21 21-4.35-4.35" />
            <path d="M10.5 18a7.5 7.5 0 1 0 0-15 7.5 7.5 0 0 0 0 15Z" />
          </svg>
          <input v-model="search" type="text" placeholder="Search">
        </label>

        <div class="popover-anchor">
          <button class="toolbar-button" :class="{ 'toolbar-button--active': filterOpen }" type="button" @click.stop="openFilter">
            <svg viewBox="0 0 24 24" fill="none">
              <path d="M4 5h16l-6 7v6l-4 1v-7L4 5Z" />
            </svg>
            <span>Filter</span>
          </button>
          <div v-if="filterOpen" class="menu-card menu-card--filter order-filter-card" @click.stop>
            <h3>Filter</h3>
            <p class="menu-label">Date</p>
            <label class="field-block field-block--compact">
              <input v-model="selectedDate" type="text" placeholder="Select Date">
            </label>
            <p class="menu-label">Type</p>
            <label class="field-block field-block--compact">
              <select v-model="typeDraft">
                <option value="All">All</option>
                <option value="Earning">Earning</option>
                <option value="Withdraw">Withdraw</option>
              </select>
            </label>
            <div class="menu-card__actions">
              <button type="button" @click="clearFilter">Clear Filter</button>
              <button type="button" @click="applyFilter">Apply Filter</button>
            </div>
          </div>
        </div>

        <div class="popover-anchor">
          <button class="toolbar-button toolbar-button--dark" type="button" @click.stop="exportOpen = !exportOpen; filterOpen = false">
            <svg viewBox="0 0 24 24" fill="none">
              <path d="M12 3v11" />
              <path d="m8 10 4 4 4-4" />
              <path d="M5 21h14" />
            </svg>
            <span>Export</span>
            <svg class="caret-icon" viewBox="0 0 24 24" fill="none">
              <path d="m6 9 6 6 6-6" />
            </svg>
          </button>
          <div v-if="exportOpen" class="menu-card" @click.stop>
            <button type="button" @click="exportCsv">Download CSV</button>
          </div>
        </div>
      </div>
    </div>

    <div v-if="selectedDateApplied || typeFilter !== 'All'" class="filter-summary">
      <span>Filtered by</span>
      <div class="filter-chips">
        <span v-if="selectedDateApplied" class="filter-chip">{{ selectedDateApplied }}</span>
        <span v-if="typeFilter !== 'All'" class="filter-chip">{{ typeFilter }}</span>
      </div>
      <button class="clear-link" type="button" @click="clearFilter">Clear all</button>
    </div>

    <div class="table-card">
      <table class="data-table">
        <thead>
          <tr>
            <th class="col-check"><input type="checkbox"></th>
            <th>Type</th>
            <th>Order Id</th>
            <th>Date</th>
            <th>Amount</th>
            <th>Balance</th>
            <th class="col-action">Action</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item in visibleTransactions" :key="item.id">
            <td class="col-check"><input type="checkbox"></td>
            <td>{{ item.type }}</td>
            <td>{{ item.orderId }}</td>
            <td>{{ item.date }}</td>
            <td><span :class="item.amount >= 0 ? 'amount-text amount-text--positive' : 'amount-text amount-text--negative'">{{ amountLabel(item.amount) }}</span></td>
            <td>GHC {{ item.balance }}</td>
            <td class="col-action">
              <button class="icon-button icon-button--table" type="button">
                <svg viewBox="0 0 24 24" fill="none">
                  <path d="M12 5.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z" />
                  <path d="M12 13.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z" />
                  <path d="M12 21.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z" />
                </svg>
              </button>
            </td>
          </tr>
        </tbody>
      </table>
    </div>

    <div class="pagination-bar">
      <div class="page-chip">20 of 240</div>
      <div class="pagination-actions">
        <button class="page-button" type="button">&lt;</button>
        <button class="page-button page-button--active" type="button">1</button>
        <button class="page-button" type="button">2</button>
        <button class="page-button" type="button">3</button>
        <button class="page-button" type="button">4</button>
        <button class="page-button" type="button">5</button>
        <button class="page-button" type="button">&gt;</button>
      </div>
    </div>
  </section>
</template>
