...
const { foods, removeFood, updateFood } = useMenu()
...
const categories = computed(() => ['All', ...new Set(foods.value.map(item => item.category))])
const statusOptions = ['All', 'Active', 'Deactive'] as const

const filteredFoods = computed(() =>
  foods.value.filter((food) => {
    const query = search.value.trim().toLowerCase()
    const matchesQuery = !query || [food.name, food.category, food.price].some(value => value.toLowerCase().includes(query))
    const matchesCategory = selectedCategory.value === 'All' || food.category === selectedCategory.value
    const matchesStatus = selectedStatus.value === 'All' || food.status === selectedStatus.value
...
const toggleFoodStatus = (id: string) => {
...
const exportRows = (type: 'csv' | 'json') => {
  const rows = filteredFoods.value
  const filename = `food-list.${type}`
  const content = type === 'json'
    ? JSON.stringify(rows, null, 2)
    : [
        ['id', 'name', 'category', 'price', 'status'].join(','),
        ...rows.map(item => [item.id, item.name, item.category, item.price, item.status].join(',')),
      ].join('\n')
  ...
  <section class="dashboard-card" @click="closeMenus">
    <div class="section-bar">
      ...
      <div class="toolbar">
        ...
        <NuxtLink class="primary-link" to="/menu/food-list/add">
          Add Food
        </NuxtLink>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="food in filteredFoods" :key="food.id">
            ...
            <td>{{ food.name }}</td>
            <td><img class="menu-thumb" :src="food.image" :alt="food.name"></td>
            <td>{{ food.category }}</td>
            <td>{{ food.price }}</td>
            <td>
              <span class="status-text" :class="food.status === 'Active' ? 'status-text--open' : 'status-text--closed'">
                {{ food.status }}
              </span>
            </td>
            <td class="col-action">
              <div class="action-anchor">
                <button class="icon-button icon-button--table" type="button" @click.stop="toggleActionMenu(food.id)">
                  <svg viewBox="0 0 24 24" fill="none">
                ...
                <div v-if="actionMenuId === food.id" class="row-menu" @click.stop>
                  <NuxtLink :to="`/menu/food-list/${food.id}`">View</NuxtLink>
                  <NuxtLink :to="`/menu/food-list/add?edit=${food.id}`">Edit</NuxtLink>
                  <button type="button" @click="removeFoodItem(food.id)">Delete</button>
                  <button type="button" @click="toggleFoodStatus(food.id)">
          ...
          <tr v-if="!filteredFoods.length">
            <td colspan="7" class="empty-state">No food items found.</td>
          </tr>
        </tbody>
      </table>
    </div>
